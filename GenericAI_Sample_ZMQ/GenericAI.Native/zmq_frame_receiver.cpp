...
struct ZmqFrameReceiver::Impl {
    ...
    Impl(std::string ep, FindChannel fc, int count)
        : endpoint(std::move(ep)), find_channel(std::move(fc)),
          channel_count(count > 0 ? count : 1) {}
    ...
    NalDecoder* DecoderFor(uint32_t channel_id, uint8_t codec) {
        ...
        if (!dec->Valid()) {
            std::cerr << "ZmqFrameReceiver: failed to build decoder for channel "
                      << channel_id << std::endl;
        }
    ...
    void Run() {
        ...
        if (zmq_connect(sock, endpoint.c_str()) != 0) {
            std::cerr << "ZmqFrameReceiver: connect failed: "
                      << zmq_strerror(zmq_errno()) << std::endl;
            return;
        }
        std::cout << "ZmqFrameReceiver: connected to " << endpoint
                  << " (channels=" << channel_count
                  << " rcvhwm=" << rcvhwm << ")" << std::endl;
        ...
        // but no [DETECT]). Printed to the wrapper console.
        ...
        while (running.load(std::memory_order_acquire)) {
            ...
            Channel* ch = find_channel ? find_channel(static_cast<int>(hdr.channel_id)) : nullptr;
            ...
            if (ch && dec && dec->Valid()) {
                ...
                std::cout << "[zmq] WARN no channel for channel_id=" << hdr.channel_id
                          << " (check the channel_id matches a wrapper port)" << std::endl;
            }
            zmq_msg_close(&nal_msg);
...
ZmqFrameReceiver::ZmqFrameReceiver(std::string endpoint, FindChannel find_channel, int channel_count)
    : impl_(new Impl(std::move(endpoint), std::move(find_channel), channel_count)) {}
...
bool ZmqFrameReceiver::Start() {
    ...
    int ioThreads = (impl_->channel_count + 15) / 16;
