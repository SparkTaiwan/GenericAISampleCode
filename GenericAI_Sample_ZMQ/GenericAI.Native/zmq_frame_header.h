...
// each frame to the Channel whose Port() == channel_id. No per-channel socket /
// port is needed.
//
// MUST stay byte-compatible with the recorder's copy at
//   spark.recorder/modules/AIService/Generic/ZmqFrameProtocol.h
#pragma pack(push, 1)
struct ZmqFrameHeader {
    ...
    uint32_t channel_id;   // routes to Channel::Port()
