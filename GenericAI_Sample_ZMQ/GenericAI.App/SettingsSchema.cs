...
namespace GenericAI.App
{
    ...
    internal static class SettingsSchema
    {
        ...
        //
        // Each field carries a `scope` (spec: settings scope) that declares WHERE it is
        // edited / stored and HOW it flows on the wire. This sample includes one field of
        // every scope as a reference:
        //   "device"  - one value for the whole device  (edited on the device page;
        //               carried in SetParameters top-level ai_settings, device-wide)
        ...
        // Rule of thumb: "what counts as a hit" (detection tuning) => roi; "how we output /
        // the whole stream" => channel; "one setting for the entire device" => device.
        private static string BuildMotion()
        {
            var schema = new
            {
                ...
                fields = new object[]
                {
                    // scope="channel": output quality, per stream (not detection tuning).
                    JpgCompressField(),

                    // scope="device" EXAMPLE: one value shared by the whole device. This
                    // motion sample does not consume it -- it is here purely to show how a
                    // device-scoped field appears in the schema and round-trips.
                    new {
                        key = "device_example", type = "int", scope = "device",
                        label       = Loc(("zh-TW", "裝置層級範例設定"), ("en", "Device-scope example")),
                        description = Loc(("zh-TW", "scope=device 示範：整台裝置共用一個值（此範例 wrapper 不使用）"), ("en", "scope=device demo: one value for the whole device (not consumed by this sample)")),
                        @default = 0, value = 0, min = 0, max = 100
                    },
        ...
        // larger payload). The wrapper clamps to <=100 and ignores <=0. Output quality,
        // not detection tuning, so it is channel-scoped.
