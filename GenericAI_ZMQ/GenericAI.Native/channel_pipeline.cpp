...
// Set once before any SetParameters (by the ZMQ receiver path). Read by every
// ApplyParameters to decide whether to spawn an MmfReader.
...
void ChannelPipeline::ApplyParameters(const GAI_Settings& s) {
    ...
    // MMF frame source: spawn the reader. ZMQ frame source: frames are pushed in
    // via SubmitDecodedFrame from the shared ZmqFrameReceiver, so no reader here.
    if (!IsZmqFrameMode()) {
...
void ChannelPipeline::SubmitDecodedFrame(const unsigned char* yuv_i420, int width,
                                         int height, int size, std::uint64_t timestamp) {
    // Not yet configured (no SetParameters) -> pool/queue absent. Drop.
    if (!has_params_.load(std::memory_order_acquire)) return;
    if (!pool_ || !infer_q_ || !yuv_i420 || size <= 0) return;

    // Mirrors MmfReader's guard: the pool slot is sized from the first
    // SetParameters resolution; a larger decoded frame is dropped rather than
    // overflowing the slot.
    if (static_cast<std::size_t>(size) > pool_->SlotCapacity()) return;

    FrameSlot* slot = pool_->Acquire();
    if (slot == nullptr) return;  // pool full -> drop (ZMQ HWM is the backpressure)

    std::memcpy(slot->data.data(), yuv_i420, static_cast<std::size_t>(size));
    slot->width     = width;
    slot->height    = height;
    slot->size      = size;
    ...
    // NON-BLOCKING: unlike MmfReader (which spin-retries to never drop), the
    // single receiver thread serves every channel, so blocking here would stall
    // them all. Drop on a full infer queue instead.
    if (!infer_q_->TryPushRef(slot)) {
        pool_->Release(slot);
        return;
    }
