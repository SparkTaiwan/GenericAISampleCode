...
namespace GenericAI.App
{
    ...
    {
        // Process-wide detector kind. Program.cs calls Configure() with the
        // authoritative kind reported by native (GAI_GetDetectorKind) right after a
        // successful init, so during normal serving this is always a concrete
        // Motion/Person. The initial value here is only a placeholder for the narrow
        // window before native init completes (HTTP listeners are already up) and for
        // degraded init where no detector loaded; it mirrors the native compile-time
        // default gai::kDetectorKind in gai_config.h (Motion) so even those edge cases
        // serve the schema for the detector that would run.
        private static int _detectorKind = (int)DetectorType.Motion;
        ...
        // (sensitivity + threshold), Person gets object detection (confidence +
        // classes). _detectorKind is a concrete kind once native init has run.
            ...
            var schema = new
            {
                ...
                fields = new object[]
                {
                    ...
                    new {
                        ...
                        scope = "roi",   // detection tuning, per region
                    ...
                    new {
                        ...
                        scope = "channel",   // output/send-rate throttle, per stream (not per ROI)
        ...
        //
        // scope (spec: settings scope) declares where a field is edited / stored and
        // how it flows on the wire:
        //   "device"  - one value for the whole device (edited on the device page)
        //   "channel" - one value per smart stream    (edited on the stream page;
        //               carried in the top-level SetParameters.ai_settings)
        //   "roi"     - one value per detection region (edited on the ROI editor;
        //               carried per entry inside SetParameters.rois[i])
        // Rule of thumb: "what counts as a hit" (detection tuning) => roi; "how we
        // output / the whole stream's behaviour" => channel. jpg_compress is output
        // quality, not detection tuning, so it is channel-scoped.
        ...
        // NOTE: draw_roi is NOT a schema field. It maps to the perimeter's built-in
        // draw_rect_on_jpg flag (Config-settable), which the recorder sends as a
        // top-level "draw_roi" and the wrapper stores in ParameterStore.DrawRoi.
    }
}
