...
// GAI_SetChannelParameters. The channel's worker thread takes a copy each
// iteration; the HTTP listener thread updates under the same mutex, so a long
// detection pass never stalls /SetParameters.
class ParamSnapshot {
public:
    struct View {
