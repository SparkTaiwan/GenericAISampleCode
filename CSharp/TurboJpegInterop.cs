...
namespace SampleWrapper
{
    internal static class TurboJpegInterop
    {
        ...
        // Callback runs on a single C++ background thread (RecognizeTask),
        // so one shared handle without a lock is safe. Switch to [ThreadStatic]
        // if callbacks ever fan out to multiple threads.
        ...
        public static byte[] EncodeI420(IntPtr yuvI420, int length, int width, int height, int quality)
        {
            ...
            uint jpegSize = 0;
            // pad=1: rows are tightly packed (no per-scanline alignment).
            // ffmpeg -pix_fmt yuv420p produces this layout.
